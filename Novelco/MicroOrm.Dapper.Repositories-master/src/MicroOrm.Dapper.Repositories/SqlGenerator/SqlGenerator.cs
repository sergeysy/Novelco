...
namespace MicroOrm.Dapper.Repositories.SqlGenerator
{
		...
		where TEntity : class
	{
		...
		public bool IsIdentity { get { return this.IdentityProperty != null; } }
		...
		public virtual QueryResult GetInsert(TEntity entity)
		{
			...
			string columNames = string.Join(", ", properties.Select(p => p.ColumnName.ToString()));
		...
		public virtual QueryResult GetSelect(Expression<Func<TEntity, bool>> expression, params Expression<Func<TEntity, object>>[] includes)
		{
			...
			for (int i = 0; i < queryProperties.Count; i++)
			{
				...
				var actualItem = BaseProperties.First(w=>w.Name == item.PropertyName);

				if (!string.IsNullOrEmpty(item.LinkingOperator) && i > 0)
				{
					builder.Append(string.Format("{0} {1}.{2} {3} @{2} ", item.LinkingOperator, TableName, actualItem.ColumnName, item.QueryOperator));
				}
				else
				{
					builder.Append(string.Format("{0}.{1} {2} @{1} ", TableName, actualItem.ColumnName, item.QueryOperator));
				}

				expando[actualItem.ColumnName] = item.PropertyValue;
			}
		...
		public virtual QueryResult GetDelete(TEntity entity)
		{
			...
			if (!LogicalDelete)
			{
				sqlBuilder.AppendFormat("DELETE FROM {0} WHERE {1}",
					this.TableName,
					string.Join(" AND ",
						this.KeyProperties.Select(p => p.ColumnName + " = @" + p.Name)));
			}
			else
			{
				sqlBuilder.AppendFormat("UPDATE {0} SET {1} WHERE {2}",
